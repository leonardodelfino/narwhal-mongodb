...
Collection.prototype = {
    ...
    aggregate : function(obj) {
    	var pipeline = new java.util.ArrayList();

    	if(obj instanceof Array) {
    		for(var i=0; i < obj.length; i++) {
        		pipeline.add(Support.createBDObject(obj[i]));
        	}
    	} else {
    		pipeline.add(obj);
    	}

		var result = this.collection.aggregate(pipeline);
    ...
    insert: function(docs) {
        var insertable = docs.isArray && docs.isArray() ?
         ...
         var writeResult = this.collection.insert(insertable);

         return {
        	 error: (writeResult.getError() ? writeResult.getError() + "" : null)
         };
    },
    update: function(criteria, obj) {
        ...
        var writeResult = this.collection.update(Support.createBDObject(criteria), Support.createBDObject(obj));
    ...
    remove: function(obj) {
    	 var writeResult = this.collection.remove(Support.createBDObject(obj));
